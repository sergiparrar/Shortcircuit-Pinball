...
// Blit to screen
bool ModuleRender::Blit(SDL_Texture* texture, int x, int y, SDL_Rect* section, float speed, double angle, SDL_Rect* dimension, int pivot_x, int pivot_y)
{
	bool ret = true;
	SDL_Rect rect;
	...
	{
		if (SDL_RenderCopyEx(renderer, texture, section, dimension, angle, p, SDL_FLIP_NONE) != 0)
		{
			LOG("Cannot blit to screen. SDL_RenderCopy error: %s", SDL_GetError());
			ret = false;
		}
